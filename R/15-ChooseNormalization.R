#' @title Decide Preprocessing Method for Single-Cell RNA-seq Data
#'
#' @description
#' Data-driven decision framework to select between SCTransform and
#' LogNormalization based on quantifiable metrics rather than heuristics.
#' Evaluates variance stabilization, data sparsity, sequencing depth
#' heterogeneity, and biological signal retention.
#'
#' @param sc A `Seurat` object or raw count matrix (`dgCMatrix`, `dgeMatrix`,
#'   `matrix`, or `data.frame` with integer counts)
#' @param subset_size Number of cells for diagnostic subsampling. If `NULL`,
#'   dynamically determined based on metadata complexity (default: 10,000 or
#'   all cells, whichever is smaller)
#' @param known_hvgs Named list of marker genes for cell types
#'   (e.g., `list(T_cell = c("CD3D", "CD3E"))`). Used to evaluate biological
#'   signal retention in highly variable genes.
#' @param batch_column Column name in meta.data indicating batch variable
#'   (e.g., "orig.ident"). Required for batch-aware diagnostics.
#' @param verbose Whether to display diagnostic messages
#' @param seed Random seed for reproducible subsampling
#'
#' @return A `list` containing:
#'   \describe{
#'     \item{recommendation}{Character: method name}
#'     \item{metrics}{`data.table` with quantified diagnostic metrics}
#'   }
#'
#' @examples
#' \dontrun{
#' # Basic usage with Seurat object
#' result <- ChooseNormalization(seurat_obj)
#' result$recommendation
#'
#' # With marker genes for refined decision
#' markers <- list(
#'   T_cell = c("CD3D", "CD3E", "CD8A"),
#'   B_cell = c("CD79A", "MS4A1")
#' )
#' result <- ChooseNormalization(seurat_obj, known_hvgs = markers)
#'
#' # Visualize diagnostic plots
#' print(result$diagnostics$var_mean_plot)
#' }
#'
#' @export
ChooseNormalization <- function(
  ...
  if (verbose) {
    cli::cli_alert_info("Comparing {length(method_objects)} method{?s}: {.pkg {method_names}}")
  }
...
metrics[, composite_score := variance_stability * (weight["variance_stability"]) +
...
ExtractMetrics <- function(
  ...
  if (!is.null(ground_truth_markers)) {
    ...
    hvgs <- head(hvgs_detected, n_hvgs)
    if (length(hvgs) > 0) {

...
two-dimensional data structures. Provides detailed positional
information including row/column indices and names when available.
Returns structured information for programmatic use while displaying
user-friendly messages.

## Usage

``` r
CheckNA(data, max_print = 5L, ...)
```
...
- data:
  ...
  A vector, matrix, data frame, or Matrix object to check for NA values.

- max_print:

  Integer. Maximum number of NA positions to display in console output.
  Set to `0` to suppress position details. Default: `5L`.
...
Invisible list containing NA information:

- `count`:

  Total number of NA values found

- `positions`:

  For vectors: integer vector of positions; For 2D data: data frame with
  row/column indices and names (if available)

- `names`:

  For named vectors: character vector of names corresponding to NA
  positions
...
- **NAs found**: Prints warning with count and up to `max_print`
  positions

- **Position details**: Includes row/column names when available for
  easier debugging

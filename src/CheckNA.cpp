...
template <typename Add> void scan_atomic(SEXP x, Add add) {
...
List check_na_vector_cpp(SEXP x) {
  ...
  return List::create(_["count"] = count, _["positions"] = wrap(pos));
}

// [[Rcpp::export]]
List check_na_dense2d_cpp(SEXP x, IntegerVector dim) {
  if (dim.size() < 2)
    stop("dim must have length >= 2.");
  int nr = dim[0];
  int nc = dim[1];
...
List check_na_sparse_csc_cpp(SEXP x, IntegerVector i, IntegerVector p,
                             IntegerVector dim) {
  if (Rf_isNull(x)) {
    return List::create(_["count"] = 0.0, _["row"] = IntegerVector(0),

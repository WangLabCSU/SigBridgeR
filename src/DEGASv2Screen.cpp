...
// [[Rcpp::export]]
Rcpp::CharacterVector get_bulk_hvg(const arma::mat &patdata,
  ...
  if (common_genes.size() != static_cast<R_xlen_t>(n_genes)) {
    Rcpp::stop(
        "The length of common_genes must equal the number of rows of patdata");
  }
  ...
  if (finite_idx.n_elem > 0) {
    sorted_finite =
        finite_idx(arma::sort_index(gene_sd.elem(finite_idx), "descend"));
  }
...
inline bool is_missing(const double x) { return std::isnan(x); }
 ...
 *   - preserves NA/NaN
 */
inline void norm_inplace_ptr(const double *x, double *out,
                             const arma::uword n) {
  double sum = 0.0;
  ...
  const double mean_value = sum / static_cast<double>(n_valid);
  ...
  if (n_valid < 2) {
    ...
    const double sd_value = std::sqrt(ss / static_cast<double>(n_valid - 1));
 ...
 */
inline void scale_ptr(const double *x, double *out, const arma::uword n) {
  ...
  const double denominator = xmax - xmin + EPS;
...
arma::vec normFunc_ptr(const arma::vec &x) {
  ...
  norm_inplace_ptr(x.memptr(), out.memptr(), x.n_elem);
  return out;
}
...
arma::vec scaleFunc_ptr(const arma::vec &x) {
  arma::vec out(x.n_elem);
  scale_ptr(x.memptr(), out.memptr(), x.n_elem);
  return out;
}
...
arma::mat normalizeScale_ptr(const arma::mat &X) {
  ...
  for (arma::uword j = 0; j < ncol; ++j) {
    const double *column_ptr = x_ptr + j * nrow;
    // step 1: column-wise normalization
    norm_inplace_ptr(column_ptr, normalized.memptr(), nrow);
    ...
    double *output_row_start = result_ptr + j;
    if (n_valid == 0) {
      for (arma::uword i = 0; i < nrow; ++i) {
      ...
      for (arma::uword i = 0; i < nrow; ++i) {
        ...
        if (is_missing(value)) {
          ...
          output_row_start[i * ncol] = (value - xmin) / denominator;
        }
      }
    }
  }
  ...
  for (arma::uword j = 0; j < ncol; ++j) {
    ...
    for (arma::uword i = 0; i < nrow; ++i) {
      ...
      if (is_missing(value)) {
        ...
        transformed[i] = std::pow(1.5, std::log2(value + 1.0));
      }
    }
    /*
     * Phase 1: normFunc
     */
    norm_inplace_ptr(transformed.memptr(), normalized.memptr(), nrow);
    /*
     * Phase 2: scaleFunc
     */
    double xmin = std::numeric_limits<double>::infinity();

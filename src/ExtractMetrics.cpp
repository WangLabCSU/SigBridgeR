...
inline bool is_na_or_nan(double x) { return ISNAN(x); }
...
// ------------------------------------------------------------
double pearson_cor(const std::vector<double> &x, const std::vector<double> &y) {
  std::size_t n = x.size();
  if (n != y.size() || n < 2)
    return NA_REAL;
...
std::vector<double> average_rank(const std::vector<double> &x) {
  ...
  while (start < n) {
    ...
    double avg_rank =
        0.5 * (static_cast<double>(start + 1) + static_cast<double>(end));
...
double spearman_cor(const arma::vec &x, const arma::vec &y) {
  ...
  for (arma::uword i = 0; i < x.n_elem; ++i) {
    double xi = x[i];
    double yi = y[i];
    if (!is_na_or_nan(xi) && !is_na_or_nan(yi) && R_FINITE(xi) &&
        R_FINITE(yi)) {
      xx.push_back(xi);
      yy.push_back(yi);
    }
...
RowStats row_stats_dgCMatrix(SEXP mat) {
  ...
  for (int r = 0; r < nr; ++r) {
    ...
    if (nc <= 1) {
      ...
      double v =
          (sumsq[r] - sum[r] * sum[r] / nc) / static_cast<double>(nc - 1);
      if (v < 0.0 && v > -1e-12)
        v = 0.0;
      var[r] = v;
    }
...
RowStats row_stats(SEXP mat) {
  ...
  stop("Unsupported assay_data matrix type. Supported: dgCMatrix, dgeMatrix, "
       "base matrix.");
}
...
DropoutStats dropout_dgCMatrix(SEXP mat) {
  ...
  for (int r = 0; r < nr; ++r) {
    if (has_na[r] || nc == 0) {
      ...
      dropout[r] =
          1.0 - static_cast<double>(detected[r]) / static_cast<double>(nc);
    }
  }
...
DropoutStats dropout_stats(SEXP mat) {
  ...
  stop("Unsupported counts matrix type. Supported: dgCMatrix, dgeMatrix, base "
       "matrix.");
}
...
// [[Rcpp::export]]
List ExtractMetricsCpp(SEXP assay_data, SEXP counts,
  ...
  if (!is_na_or_nan(q)) {
    ...
    for (int g = 0; g < rs.nrow; ++g) {
      double mu = rs.mean[g];
      double va = rs.var[g];
      if (!is_na_or_nan(mu) && !is_na_or_nan(va) && mu > q && va > 0.0) {
        n_valid_basic++;
  ...
  return List::create(_["variance_mean_cor"] = variance_mean_cor,
                      _["mean_dropout_residual"] = mean_dropout_residual,
                      _["n_cells"] = rs.ncol, _["n_genes"] = rs.nrow);
}
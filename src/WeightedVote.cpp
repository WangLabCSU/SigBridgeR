...
struct TieRandom {
  static inline int choose() { return R::unif_rand() < 0.5 ? 0 : 1; }
};

struct TieFirst {
  static inline int choose() { return 0; }
};

struct TieLast {
  static inline int choose() { return 1; }
};

template <typename TiePolicy>
CharacterVector weighted_vote_impl(CharacterMatrix vote_data,
...
// [[Rcpp::export()]]
CharacterVector weighted_vote_cpp(CharacterMatrix vote_data,
                                  NumericVector weights, int ties_method) {
  Rcpp::RNGScope scope;
  switch (ties_method) {
  case 0:

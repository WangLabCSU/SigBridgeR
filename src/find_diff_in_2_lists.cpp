...
static inline bool is_list_cpp(SEXP x) { return TYPEOF(x) == VECSXP; }

static inline SEXP no_diff() { return Rf_ScalarLogical(1); }

static inline bool is_no_diff(SEXP x) {
  return TYPEOF(x) == LGLSXP && XLENGTH(x) == 1 && LOGICAL(x)[0] == 1;
}

static inline SEXP diff_pair(SEXP x, SEXP y, SEXP name) {
  return List::create(_["name"] = name, _["x"] = x, _["y"] = y);
}

static inline bool has_names(SEXP x) {
  SEXP nms = Rf_getAttrib(x, R_NamesSymbol);
  return nms != R_NilValue && TYPEOF(nms) == STRSXP &&
    ...
    if (use_names) {
      ...
      for (R_xlen_t i = 0; i < nx; ++i) {
        ...
        if (nm.empty()) {
          ...
          SEXP ans =
              find_diff_rec(VECTOR_ELT(x, i), VECTOR_ELT(y, i), current_name);
          if (!is_no_diff(ans))
            return ans;
          continue;
        }
        ...
        SEXP ans =
            find_diff_rec(VECTOR_ELT(x, i), VECTOR_ELT(y, it->second), nm_sexp);
        if (!is_no_diff(ans))
          return ans;
      }
      for (R_xlen_t j = 0; j < ny; ++j) {

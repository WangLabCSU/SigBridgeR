...
2. The output must be a **list** containing at least one elements:
   - `scRNA_data` (required): A **Seurat** object with meta.data modified,
     ...
     - If you need to store the parameters used for running the algorithm, use the `Seurat@misc` slot and store them as a **list**, e.g., `Scissor_params = list(alpha = 0.05, cutoff = 0.2)`.

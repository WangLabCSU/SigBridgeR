...
your_seurat <- SCPreProcess(
  ...
  scale_factor = 10000,
  # When ` scale_features = "hvg"`, high-variance genes via `VariableFeatures()` are used.
  scale_features = NULL,
  selection_method = "vst",
  resolution = 0.6,   
  # When specify `NULL`, `dims` will be auto-selected by `FindRobustElbow`
  dims = 1:10,
  dims_Neighbors = dims,
  run_TSNE = TRUE, # whether to run `RunTSNE`
  dims_TSNE = dims,
  dims_UMAP = dims,
  verbose = TRUE,
  ...
  # A meta data column name for binary label (Tumour, Normal)
  column2only_tumor = NULL,
  # use `SCTransform` instead of `NormalizeData` + `FindVariableFeatures` + `ScaleData`
  use_sctransfrom = FALSE,
  ...
)
```
